 ...
 *      working process is consistent with the STAR pipeline.
 * 
 * INHERITANCES:
 * 
 * 
 ...
 *          Basis of the pose-dependent shape space, (6890, 3, 93).
 * 
...
public: // PUBLIC METHODS
    ...
    // %% Constructor and Destructor %%
    STAR() noexcept(true);
    STAR(std::string &modelPath,
        std::string &vertPath, torch::Device &device) noexcept(false);
    STAR(const STAR& smpl) noexcept(false);
    ~STAR() noexcept(true);

    // %% Operators %%
    STAR &operator=(const STAR& smpl) noexcept(false);
